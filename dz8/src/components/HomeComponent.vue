<template>
    <section class="banner center">
        <div class="banner__info">
            <h1 class="banner__heading">Let Your Home Be Unique</h1>
            <p class="banner__text">There are many variations of the passages of
                lorem Ipsum fromavailable, majority.</p>
            <router-link class="banner__button" to="/not-page">Get Started
                <svg width="19" height="17" viewBox="0 0 19 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M2 8.43542L15.7232 8.29857M10.6182 1.91138L17.1412 8.43436L10.4868 15.0887" stroke="#CDA274"
                        stroke-width="2" stroke-linecap="square" stroke-linejoin="round" />
                </svg></router-link>
        </div>
        <div>
            <img class="banner__img" src="@/assets/img/banner.png" alt="banner_photo">
        </div>
    </section>
    <section class="project center">
        <h2 class="project__heading">Follow Our Projects</h2>
        <p class="project__text">It is a long established fact that a reader will be distracted by the of readable
            content of page lookings at its layouts points.</p>
        <div class="project__items">
            <div class="project__item">
                <img class="project__item__img  project__item__img-top-right" src="@/assets/img/project__item1.png"
                    alt="project1">
                <div class="project__item__content-box">
                    <div class="project__item__content">
                        <h3 class="project__item__title">Modern Kitchan</h3>
                        <p class="project__item__text">Decor / Artchitecture</p>
                    </div>
                    <div><router-link to="/project"><svg width="70" height="70" viewBox="0 0 70 70" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <circle cx="35" cy="35" r="35" fill="#F4F0EC" />
                                <path d="M32 44L40 35L32 26" stroke="#292F36" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round" />
                            </svg></router-link></div>
                </div>

            </div>
            <div class="project__item">
                <img class="project__item__img project__item__img-top-left" src="@/assets/img/project__item1.png"
                    alt="project1">
                <div class="project__item__content-box">
                    <div class="project__item__content">
                        <h3 class="project__item__title">Modern Kitchan</h3>
                        <p class="project__item__text">Decor / Artchitecture</p>
                    </div>
                    <div><router-link to="/project"><svg width="70" height="70" viewBox="0 0 70 70" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <circle cx="35" cy="35" r="35" fill="#F4F0EC" />
                                <path d="M32 44L40 35L32 26" stroke="#292F36" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round" />
                            </svg></router-link></div>
                </div>

            </div>
            <div class="project__item">
                <img class="project__item__img project__item__img-bottom-right" src="@/assets/img/project__item1.png"
                    alt="project1">
                <div class="project__item__content-box">
                    <div class="project__item__content">
                        <h3 class="project__item__title">Modern Kitchan</h3>
                        <p class="project__item__text">Decor / Artchitecture</p>
                    </div>
                    <div><router-link to="/project"><svg width="70" height="70" viewBox="0 0 70 70" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <circle cx="35" cy="35" r="35" fill="#F4F0EC" />
                                <path d="M32 44L40 35L32 26" stroke="#292F36" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round" />
                            </svg></router-link></div>
                </div>

            </div>
            <div class="project__item">
                <img class="project__item__img project__item__img-bottom-left" src="@/assets/img/project__item1.png"
                    alt="project1">
                <div class="project__item__content-box">
                    <div class="project__item__content">
                        <h3 class="project__item__title">Modern Kitchan</h3>
                        <p class="project__item__text">Decor / Artchitecture</p>
                    </div>
                    <div><router-link to="/project"><svg width="70" height="70" viewBox="0 0 70 70" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <circle cx="35" cy="35" r="35" fill="#F4F0EC" />
                                <path d="M32 44L40 35L32 26" stroke="#292F36" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round" />
                            </svg></router-link></div>
                </div>

            </div>
        </div>
    </section>
    <div class="info-counter center">
        <article class="info-counter__item">
            <p class="info-counter__item_number">12</p>
            <p class="info-counter__item_text">Years Of Experiance</p>
        </article>

        <article class="info-counter__item">
            <p class="info-counter__item_number">85</p>
            <p class="info-counter__item_text">Success Project</p>
        </article>
        <article class="info-counter__item">
            <p class="info-counter__item_number">15</p>
            <p class="info-counter__item_text">Active Project</p>
        </article>
        <article class="info-counter__item">
            <p class="info-counter__item_number">95</p>
            <p class="info-counter__item_text">Happy CUstomers</p>
        </article>
    </div>
    <section class="articles center">
        <h2 class="articles__heading">Articles & News</h2>
        <p class="articles__text">It is a long established fact that a reader will be distracted by the of readable
            content of a page when
            lookings at its layouts the points of using.</p>
        <div class="articles__item-wrap">
            <article class="articles__item">
                <img class="articles__item__img" src="@/assets/img/articles__item1.png" alt="project1">
                <div class="articles__item__teg">
                    <p class="articles__item__teg__text">Kitchan Design</p>
                </div>
                <div class="articles__item__content">
                    <h3 class="articles__item__title">Let’s Get Solution For Building Construction Work</h3>
                    <div class="articles__item__date">
                        <p class="articles__item__text">26 December,2022 </p>
                        <router-link to="/blog"><svg width="52" height="53" viewBox="0 0 52 53" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <circle cx="26" cy="26.267" r="26" fill="#F4F0EC" />
                                <path d="M23.7714 32.9527L29.7143 26.267L23.7714 19.5813" stroke="#292F36" stroke-width="2"
                                    stroke-linecap="round" stroke-linejoin="round" />
                            </svg></router-link>
                    </div>
                </div>
            </article>
            <article class="articles__item">
                <img class="articles__item__img" src="@/assets/img/articles__item2.png" alt="project1">
                <div class="articles__item__teg">
                    <p class="articles__item__teg__text">Living Design</p>
                </div>
                <div class="articles__item__content">
                    <h3 class="articles__item__title">Low Cost Latest Invented Interior Designing
                        Ideas.</h3>
                    <div class="articles__item__date">
                        <p class="articles__item__text">22 December,2022 </p>
                        <router-link to="/blog"><svg width="52" height="53" viewBox="0 0 52 53" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <circle cx="26" cy="26.267" r="26" fill="#F4F0EC" />
                                <path d="M23.7714 32.9527L29.7143 26.267L23.7714 19.5813" stroke="#292F36" stroke-width="2"
                                    stroke-linecap="round" stroke-linejoin="round" />
                            </svg></router-link>
                    </div>
                </div>
            </article>
            <article class="articles__item">
                <img class="articles__item__img" src="@/assets/img/articles__item3.png" alt="project1">
                <div class="articles__item__teg">
                    <p class="articles__item__teg__text">Interior Design</p>
                </div>
                <div class="articles__item__content">
                    <h3 class="articles__item__title">Best For Any Office & Business Interior
                        Solution</h3>
                    <div class="articles__item__date">
                        <p class="articles__item__text">25 December,2022 </p>
                        <router-link to="/blog"><svg width="52" height="53" viewBox="0 0 52 53" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <circle cx="26" cy="26.267" r="26" fill="#F4F0EC" />
                                <path d="M23.7714 32.9527L29.7143 26.267L23.7714 19.5813" stroke="#292F36" stroke-width="2"
                                    stroke-linecap="round" stroke-linejoin="round" />
                            </svg></router-link>
                    </div>
                </div>
            </article>
        </div>

    </section>
</template>

<script>
export default {
    name: "HomeComponent",
}
</script>

<style lang="scss" scoped>
@import "@/assets/_vars.scss";

.banner {
    position: relative;
    max-width: 1201px;
    height: 758px;

    &__info {
        position: absolute;
        padding: 208px 0 208px 53px;
        width: 434px;
        display: flex;
        flex-direction: column;
        align-items: flex-start;
    }

    &__heading {
        color: $siteColorHeading;
        font-size: 65px;
        font-family: DM Serif Display;
        font-weight: 400;
        line-height: 81.25px;
        word-wrap: break-word;
    }

    &__text {
        width: 433px;
        color: $siteColorText;
        font-size: 22px;
        font-family: Jost;
        font-weight: 400;
        line-height: 33px;
        letter-spacing: 0.22px;
        word-wrap: break-word;
        padding: 18px 0 21px;
    }

    &__img {
        background: #F4F0EC;
        border-radius: 70px;
        width: 100%;
    }

    &__button {
        display: flex;
        padding: 26px 49px 26px 48px;
        flex-direction: row;
        justify-content: center;
        align-items: center;
        gap: 10px;
        border-radius: 18px;
        background: #292F36;
        box-shadow: 0px 10px 20px 0px rgba(192, 192, 192, 0.35);
        color: #FFF;
        text-align: center;
        font-family: Jost;
        font-size: 18px;
        font-style: normal;
        font-weight: 600;
        line-height: 22.5px;
        letter-spacing: 0.36px;
    }
}

.project {
    &__heading {
        color: $siteColorHeading;
        text-align: center;
        font-family: DM Serif Display;
        font-size: 50px;
        font-style: normal;
        font-weight: 400;
        line-height: 62.5px;
        letter-spacing: 1px;
        padding-top: 96px;
        padding-bottom: 8px;
    }

    &__text {
        color: $siteColorText;
        text-align: center;
        font-family: Jost;
        font-size: 22px;
        font-style: normal;
        font-weight: 400;
        line-height: 33px;
        letter-spacing: 0.22px;
        padding-bottom: 93px;
    }

    &__items {
        display: flex;
        flex-wrap: wrap;
        row-gap: 56px;
        column-gap: 104px;
        padding-bottom: 96px;
    }

    &__item {
        display: flex;
        flex-direction: column;
        width: 548px;
        align-items: flex-start;
        gap: 24px;


        &__img {
            background: #C4C4C4;
            width: 100%;
            height: 548px;

            &-top-right {
                border-radius: 0px 80px 0px 0px;
            }

            &-top-left {
                border-radius: 80px 0px 0px 0px;
            }

            &-bottom-right {
                border-radius: 0px 0px 80px 0px;
            }

            &-bottom-left {
                border-radius: 0px 0px 0px 80px;
            }
        }

        &__content-box {
            display: flex;
            flex-direction: row;
            align-items: center;
            justify-content: space-between;
            width: 100%;
        }

        &__title {
            color: $siteColorHeading;
            font-family: DM Serif Display;
            font-size: 25px;
            font-style: normal;
            font-weight: 400;
            line-height: 31.25px;
            letter-spacing: 0.5px;
        }

        &__text {
            color: $siteColorText;
            font-family: Jost;
            font-size: 22px;
            font-style: normal;
            font-weight: 400;
            line-height: 33px;
            letter-spacing: 0.22px;
        }
    }
}

.info-counter {
    display: flex;
    justify-content: center;
    gap: 76px;
    background-color: #F4F0EC;
    height: 457px;
    align-items: center;

    &__item {
        text-align: center;
    }

    // &__item:not(:last-child)::after {
    //     content: '';
    //     display: inline-block;
    //     width: 1px;
    //     height: 130px;
    //     background: #CDA274;
    // }

    &__item_number {
        color: $siteColor;
        font-family: DM Serif Display;
        font-size: 85px;
        font-style: normal;
        font-weight: 400;
        line-height: 106.25px;
        letter-spacing: 1.7px;
    }

    &__item_text {
        color: #4D5053;
        font-family: Jost;
        font-size: 22px;
        font-style: normal;
        font-weight: 400;
        line-height: 33px;
        letter-spacing: 0.22px;
    }

}

.articles {
    margin: 96px 0 96px;

    &__heading {
        color: $siteColorHeading;
        text-align: center;
        font-family: DM Serif Display;
        font-size: 50px;
        font-style: normal;
        font-weight: 400;
        line-height: 62.5px;
        letter-spacing: 1px;
    }

    &__text {
        color: $siteColorText;
        text-align: center;
        font-family: Jost;
        font-size: 22px;
        font-style: normal;
        font-weight: 400;
        line-height: 33px;
        letter-spacing: 0.22px;
        padding-top: 12px;
        padding-bottom: 50px;
    }

    &__item-wrap {
        display: flex;
        flex-direction: row;
        justify-content: space-between;
        flex-wrap: wrap;
    }

    &__item {
        position: relative;
        width: 340px;
        height: 479px;
        padding: 21px;
        box-shadow: 0px 10px 30px rgba(255, 255, 255, 0.25);
        border-radius: 62px;
        border: 0.50px #E7E7E7 solid;

        &__img {
            border-radius: 45px 45px 0px 0px;
            background: #EAEAEA;
        }

        &__teg {
            position: absolute;
            display: flex;
            align-items: center;
            justify-content: center;
            left: 40px;
            top: 250px;
            width: 124px;
            height: 41px;
            border-radius: 8px 8px 8px 0px;
            background: #FFF;

            &__text {
                color: $siteColorText;
                font-family: Jost;
                font-size: 16px;
                font-style: normal;
                font-weight: 400;
                line-height: 24px;
                letter-spacing: 0.16px;
                text-transform: capitalize;
            }
        }

        &__title {
            display: flex;
            width: 305px;
            height: 70px;
            flex-direction: column;
            justify-content: center;
            color: $siteColorHeading ;
            font-family: DM Serif Display;
            font-size: 25px;
            font-style: normal;
            font-weight: 400;
            line-height: 31.25px;
            letter-spacing: 0.5px;
            padding-top: 20px;
            padding-bottom: 30px;
        }

        &__date {
            display: flex;
            flex-direction: row;
            align-items: center;
            justify-content: space-between;
        }

        &__text {
            color: $siteColorText;
            font-family: Jost;
            font-size: 16px;
            font-style: normal;
            font-weight: 400;
            line-height: 24px;
            letter-spacing: 0.16px;
            text-transform: capitalize;
        }
    }
}</style>